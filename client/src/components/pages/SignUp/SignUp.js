...
function SignUp({ classes, error, signUp, clearErrors, isAuthenticated }) {
	...
	const handleInput = event => {
		...
		setRegister({
			...register,
			[event.target.name]: event.target.type === 'checkbox' ? !agreement : event.target.value,
		});
	};
	...
	const { firstName, lastName, email, password, confirmPassword, phone, job, country, agreement } = register;
		...
		<div className={classes.signUp}>
			...
			<Paper elevation={1} className={classes.signUpArea}>
				...
				<div className={classes.signOutFooter}>
					<Typography component="p" variant="body2" className={classes.signOutFooterContent}>
						Already have an account.
						<Link component={RouterLink} to="/users/signin" color="secondary" className={classes.loginLink}>
							Login
						</Link>
					</Typography>
				</div>
